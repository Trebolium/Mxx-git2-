...
def plot_save_feature(feature, fname):
    fname = fname[:-4]
    plt.figure(figsize=(10, 4))
    plt.imshow(feature, aspect='auto', origin='lower')
    plt.title(fname)
    plt.colorbar()
    plt.savefig('jamendo/imageTrain/' +fname +'.png')
    plt.close(fname)
    ...
    while 1:
        ...
        for i in range(num_steps):  # one loop per epoch
                ...
                for j in range(params['batch_size']):
                    ...
                    random_excerpt_index = random.randint(0,song_num_frames-(params['sample_frame_length']-1))
                    # sample_excerpt must be a slice from the feature numpy
                    sample_excerpt = feature[:,random_excerpt_index:random_excerpt_index+params['sample_frame_length']]
                    #calculate the time it spans and compare this to the songLength lists
                    ...
                    # if previous entry is odd, there are vocals (1). Else no vocals (0)
                    random_excerpt_index
                    label_points=hdf5_file[label_dataset][inds[f_ind], ...]

                    #from song length, determine if label is singing or not

                    x_data.append(feature)
                    label = hdf5_file[label_dataset][inds[f_ind], ...]
                    y.append(label)
                # convert to arrays
